...
### Bucket 1A — Resolved 2026-04-26 (story CSS-7)

Eight outer dashboard panels (`.dt-snapshot-panel`, `.dt-scene-panel`, `.dt-matrix-panel`, `.dt-conflict-panel`, `.dt-chk-panel`, `.dt-inv-panel`, `.proc-amb-dashboard`, `.proc-phase-section`) plus `.proc-attach-panel` (with outer-padding exception) collapsed into a canonical outer-panel grouped selector at `public/css/admin-layout.css:1360-1374` carrying `border: 1px solid var(--bdr); border-radius: 6px; overflow: hidden; margin-bottom: 12px;`. Seven collapsible toggle headers (`.dt-snapshot-toggle`, `.dt-scene-toggle`, `.dt-chk-toggle`, `.dt-matrix-toggle`, `.proc-phase-header`, `.proc-amb-header`, `.proc-disc-header`) collapsed into the canonical loud-header grouped selector at lines 1376-1403. The "quiet" toggle tier is retired — three previously-plain headers (snapshot, scene, checklist) now render with `--accent` 600 weight + border-bottom. `.proc-amb-dashboard`, `.proc-phase-section`, `.proc-attach-panel` previously had their own backgrounds (`--surf1`/`--surf`); now transparent like peers. `.dt-chk-panel` 4px radius collapsed to canonical 6px. No JS edits, no class renames. Net −65 LOC (8244 → 8179). Visual verification pending user action.
