...
**Branch:** ms/issue-350-ordeals-submit-audit (deleted 2026-08-18, local + origin — superseded)
**Status:** superseded
**Date:** 2026-05-18

---

> ## ✅ SUPERSEDED — 2026-08-18, Angelus's own ruling: redundant given TM Admin/Cockpit's process
>
> This story was picked up from a stranded branch as part of a wider stranded-branch cleanup sweep
> (found alongside issue-779 and issue-791, both of which WERE reconciled onto `dev` the same
> session — see `specs/stories/fix.779.contacts-pt-merit-free-sum.story.md` and
> `specs/stories/fix.791.dt-proc-roller-label-spec-bonus.story.md`). Independent investigation at
> the time found the story's own premise (player-submission reliability, a read-only
> `audit-player-records.js` script, and documentation of the two-pipeline `ordeal_type` split) was
> **not** superseded by the earlier 2026-08-18 "ordeals are now owned by Story" ruling that closed
> `ord-10`/`ord-11` — that ruling covers ST-side rubric authoring and the archive-refine action
> specifically, and this branch touches neither. The silent-403 bug it targets (a player with no
> `players` record sees blank ordeal cards instead of a clear message) was confirmed still live on
> `dev` at investigation time, and its own Dev Agent Record below claims a separate,
> production-breaking bug fix (a schema/body field mismatch that made every ordeal-response POST
> fail with 400). Checked before closing: that specific bug is **already fixed on `dev`**,
> independently, via a different issue (#525) — `server/schemas/ordeal.schema.js`'s
> `ordealResponseSchema.required` is `['type']`, not `['ordeal_type']`, with the schema's own
> comment explaining exactly this mismatch. No live P0 left unaddressed by closing this branch.
>
> Closed instead on Angelus's own direct, more specific ruling: **this whole surface is redundant
> given how TM Admin and TM Cockpit now process ordeals/downtime** — not re-derived from any
> scoping doc, told directly. Not built here. If any piece of this (the player-facing error state,
> the player/character-record audit tooling, or the claimed POST-validation fix) is still needed,
> it belongs to whichever of TM Admin/TM Cockpit now owns that surface, not this repo. Branch
> deleted, local + origin, after this note was committed. The rest of this file is preserved
> unmodified below as the paper trail of what the branch intended and claimed.
