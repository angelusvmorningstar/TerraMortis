...
The design system validated in `public/mockups/font-test.html` uses three fonts only and a set of semantic colour tokens that are theme-aware. All 11 subsequent stories (DS-02 through DS-12) depend on these tokens being present in `theme.css` before any sweep begins.
