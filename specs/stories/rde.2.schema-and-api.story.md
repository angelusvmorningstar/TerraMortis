---
title: 'Rule-engine schemas and CRUD API for six typed collections'
type: 'feature'
created: '2026-04-28'
...
**Approach:** Add JSON Schema files under `server/schemas/rules/` (one per collection), wire them through the existing `validate()` middleware, and create six Express routers under `/api/rules/<family>` with standard list / get / create / update / delete handlers. ST-only auth at the router level. No business logic beyond shape validation; rule evaluation lives in the runtime evaluator added by per-family migration stories.

## Boundaries & Constraints

**Always:**
- Six collections per ADR catalogue: `rule_grant`, `rule_skill_bonus`, `rule_nine_again`, `rule_disc_attr`, `rule_derived_stat_modifier`, `rule_tier_budget`.
...
- [ ] `server/schemas/rules/rule-tier-budget.schema.js` (new) — Ajv schema. Fields: `source` (string, e.g. `'MCI'`), `budgets` (array of integers, indexed by tier where index 0 = unused), `notes`, timestamps.
- [ ] `server/routes/rules-engine.js` (new) — exports six routers, one per collection. Each: list / get / create / update / delete. Uses `validate()` middleware on POST/PUT.
- [ ] `server/index.js` — mount the six routers under `/api/rules/grant`, `/api/rules/skill_bonus`, `/api/rules/nine_again`, `/api/rules/disc_attr`, `/api/rules/derived_stat_modifier`, `/api/rules/tier_budget`. All gated `requireAuth` + `requireRole('st')`.
- [ ] `server/tests/helpers/test-app.js` — mirror the six mounts.
- [ ] `server/tests/api-rules-engine.test.js` (new) — six describe blocks, one per collection. Cover: ST creates valid doc, ST sends extra field 400s, player blocked, list empty, update missing 404, delete works. Lean on the I/O Matrix.
