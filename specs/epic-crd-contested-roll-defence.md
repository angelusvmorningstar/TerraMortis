...
| crd.4a | City Status/BP defensive half (at-Court advantage) | **done** | Storied, dev-storied, and code-reviewed 2026-08-26 (external Codex, 3 patches applied and prove-discriminated: an attacker-writable `defender_status_term` via POST closed, a non-finite pool total no longer persists as `NaN`, session-selection no longer shadowable by a future-dated session). One reviewer-labelled "High" race dismissed with evidence as pre-existing (matches `deferred-work.md`'s own already-analysed TOCTOU entry for this route, which already judged the resolve-vs-resolve case safe). Full record: `specs/stories/crd-4a-defensive-status-choice.md`. **crd.4b (attack-penalty half) WITHDRAWN 2026-08-26 — Angelus ruled it was his own error; City Status only ever applies on defence. This closes the whole crd.4 house rule and this epic — see below.** | **Defensive half ruled by Angelus 2026-08-26** (Errata still not updated; players already informed of the change, so the ruling stands ahead of the citation): when defending against a power, at court, with game mode active, if the defender's City Status is higher than the attacker's, the defender is offered an explicit choice between their normal BP-based term and the City Status difference term — best-of is computed and shown, but the player always picks, never auto-selected even when one option is mathematically clear. Scope confirmed **narrow**: this is a gated substitution for this specific context only, and does **not** change the generic Defensive Reaction pool crd.1–3b already ships (that pool still uses Resistance Attribute + Willpower + merits only, no Blood Potency/City Status term, everywhere outside this gated context). **Attack-penalty half — WITHDRAWN 2026-08-26, not merely deferred:** Angelus corrected the record: "City Status difference penalising the attack is incorrect on my part, only the CS as defence is valid." There is no attack-penalty mechanic anywhere in this house rule; City Status only ever contributes on the DEFENDER's side, exactly as crd.4a implements, and no crd.4b will be created. Of Mary's original four edge-case questions, attacker-higher-status is now fully answered by this withdrawal (there is no attacker-side effect to define at all), and City-Status ties are moot for the defensive substitution (a zero difference just leaves BP as the better/only real choice). **BP=0/gap=0 floor — CLOSED, moot (2026-08-26):** there is no BP-0 player in this game. Thin-bloods are a Masquerade concept, not used in this Requiem game; every vampire PC has BP ≥1. Mortals have neither Blood Potency nor City Status at all, so they are not eligible participants in this mechanic in the first place — no floor logic is needed. **Capped-vs-uncapped scaling — CLOSED, ruled uncapped (2026-08-26):** the City-Status-difference term scales freely, exactly as computed (defender's effective City Status minus attacker's, both already individually capped at 10 by `calcEffectiveCityStatus`) — no separate cap on the difference itself. Matches how Blood Potency is already treated raw/uncapped elsewhere in this codebase's defensive pools (`public/js/shared/resist.js`); no special-casing this new term. **The defensive half is now fully scoped** — nothing left blocking it except that a story hasn't been written yet. **"At court, game mode active" detection — resolved 2026-08-26, no new plumbing needed:** "game mode active" is the existing Chapter `phase === 'game'` signal (CM-1, `chapters.phase`); "at court" for a given character is the same attendance check the downtime form already uses — that character's `attended: true` row in the live session's `game_sessions.attendance` array. Both signals already exist server-side; crd.4's defensive-half story reads them, it does not need to invent them. **Design-locked 2026-08-26 (Sally):** `public/mockups/crd-4-defensive-status-choice-mockup.html`, Playwright-verified in both themes. New "City Status advantage" section sits between Willpower and Merits on the existing crd-3b resolve screen; a 2-option exclusive choice (`.cr-status-seg`, scoped separately from but visually matching `.cr-aspect-seg`'s family) between the character's Blood Potency term and the City-Status-difference term, each showing its computed dice value, with a small "Higher" pill as guidance only — **neither option is pre-selected**, matching the ruling that the player always picks even when one value is obviously larger. Until a term is chosen the pool preview shows a plain-label placeholder ("Choose above", explicitly NOT set in the display font per this project's Cinzel-is-numerals-only rule — caught and fixed during design-lock) and the Roll button is labelled "Choose a status term first" (not actually disabled — corrected during dev-story to match crd-3b's own no-client-duplicate-guard precedent). Selected-state styling reuses the same `.on` tint treatment as every other control on the screen (Angelus's call: consistency over a stronger bespoke affordance). Does not block crd.1–3b, which are unaffected either way. |
...
- crd.4 needs its own dedicated scoping pass (Mary's four open questions) before any story is
  written against it — do not fold it into crd.1–3b's dev-story work. **Resolved and CLOSED
  2026-08-26**: see crd.4a's own table row above for the defensive-half ruling, design-lock,
  story, dev-story, and code review, all done the same day. **crd.4b (the attack-penalty half)
  is WITHDRAWN, not just paused** — Angelus corrected the record 2026-08-26: "City Status
  difference penalising the attack is incorrect on my part, only the CS as defence is valid."
  There is no attack-penalty mechanic; City Status only ever contributes on the DEFENDER's side,
  exactly as crd.4a already implements. crd.4b will not be created. **This closes the entire
  crd.4 house rule and, with it, this whole epic** — nothing further is scoped or pending.
