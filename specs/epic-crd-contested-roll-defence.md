...
- crd.4 needs its own dedicated scoping pass (Mary's four open questions) before any story is
  written against it — do not fold it into crd.1–3b's dev-story work. **Partially resumed
  2026-08-26**: see crd.4's own table row above for the defensive-half ruling now confirmed, and
  the "at court, game mode active" detection question, now resolved (existing `chapters.phase` +
  `game_sessions.attendance` signals, no new plumbing). BP=0/gap=0 floor is closed as moot (no
  BP-0 player exists in this game). Capped-vs-uncapped scaling ruled 2026-08-26: **uncapped**.
  **The defensive half is now fully scoped.** Still open before a full crd.4 story (both halves):
  only the attack-penalty half, which Angelus is holding pending a check with Symon. The defensive
  half alone is ready to design-lock/story on its own right now.
