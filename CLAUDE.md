...
- **Epic BL** (#1008) moved bloodlines out of `public/js/data/constants.js` (`BLOODLINE_DISCS`, `BLOODLINE_CLANS`, `APPROVED_BLOODLINES`, all deleted in BL-3b) into the `bloodlines` MongoDB collection. Live reads go through `public/js/data/bloodlines-cache.js` (`approvedBloodlines`, `bloodlinesByClan`, `discsForBloodline`; refetches on the bloodline WS frame), and `clanDiscList`/`isInClanDisc` in `public/js/data/accessors.js` are the only in-clan implementation. Server-side: `server/routes/bloodlines.js` (public `GET /` only). **ADMR-1 retired ST authoring from this repo** — admin CRUD (create/edit/delete, and the impact-preview screen) now lives entirely in TM Admin, a separate app writing to this same shared `bloodlines` collection; do not re-add a write route or an admin screen here without a fresh ADR-style decision. The one-time migration is retired at `server/scripts/archive/seed-bloodlines.js`, which carries the 23 original bloodlines as frozen literals; do not edit them, add a bloodline via TM Admin.
