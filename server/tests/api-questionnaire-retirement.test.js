/**
 * API tests — /api/questionnaire, the Ordeals retirement gate.
 *
 ...
 * still edit or submit an EXISTING draft via PUT /:id (6 non-approved
 * questionnaire_responses were live and reachable this way when the gap was
...
afterEach(async () => {
  await getCollection('questionnaire_responses').deleteMany({ character_id: { $in: [CHAR_OID, CHAR_ID] } });
});
...
describe('POST /api/questionnaire — Ordeals retired', () => {
...
describe('PUT /api/questionnaire/:id — Ordeals retired', () => {
  it('403s a player update — Ordeals retired, file on the sibling site', async () => {
    ...
    const res = await request(app)
      .put(`/api/questionnaire/${created.body._id}`)
      .set('X-Test-User', playerUser([CHAR_ID], { player_id: PLAYER_ID }))
