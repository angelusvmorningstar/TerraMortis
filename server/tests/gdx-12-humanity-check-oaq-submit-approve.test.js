...
// gdx.12's own "app.js Load Pool banner is v2-gated (AC8, static-analysis)"
// describe block was removed during the dev->main reconciliation (2026-08-25):
// its whole premise (a v1/v2 roller split, gated by the tm-use-new-dice-roller
// flag) no longer exists — rlv.2 retired roll.js and the flag outright, so
// roll-v2.js is unconditionally the only player roller. checkForResolvedHumanityCheck()
// is called unconditionally now, which is the CORRECT behaviour post-rlv.2, not
// a regression of AC8 (which itself only made sense while two rollers existed).
