 ...
 *   - fact has no `fact_key`  -> $set exactly that one field to a fresh
 *                                `randomUUID()`.
 *   - fact already has one    -> NEVER touched. Never overwritten, never
 ...
 * Each write is per-index and filtered on
 * `{ 'facts.N.fact_key': { $exists: false } }`, which makes it self-guarding:
 * a no-op if that slot gained a key since planning, and a single-field $set
 * so it can never clobber a concurrent edit to any other field of that fact.
 *
 ...
 * write decisions are re-derived from that same fresh read, not from the
 * possibly-stale plan. Idempotent: re-running `planBackfill` after a
 * successful `--apply` returns an empty array.
 *
...
export async function planBackfill(collection) {
  ...
  for (const doc of docs) {
    ...
    facts.forEach((fact, i) => {
      ...
      if (!Object.prototype.hasOwnProperty.call(fact, 'fact_key')) indices.push(i);
    });
...
export async function applyBackfill(collection, rows, { apply = false, log = () => {} } = {}) {
  if (!rows.length) {
    log('Nothing to backfill - every fact already carries a fact_key.');
  ...
  // here) - the same pattern DBO-1's and DBO-8's own reviews established. The
  // per-index `$exists: false` filter on each update is a second, DB-level
  // guard against the same gap between this re-read and the write itself, so
  // a slot that gained a key out of band is silently skipped rather than
  // re-minted.
  ...
  for (const row of rows) {
    ...
    for (const i of row.indices) {
      const fact = facts[i];
      // Index no longer exists, or already carries a key as of the fresh read.
      if (!fact || typeof fact !== 'object') continue;
      if (Object.prototype.hasOwnProperty.call(fact, 'fact_key')) continue;

      const result = await collection.updateOne(
        { _id: row._id, [`facts.${i}.fact_key`]: { $exists: false } },
