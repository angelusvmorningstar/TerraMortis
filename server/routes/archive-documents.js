...
// Body JSON: { character_id, type, title?, content_html?, visible_to_player? }
router.post('/', requireRole('st'), async (req, res) => {
  const { character_id, type, title, content_html, visible_to_player } = req.body || {};

  const ALLOWED_TYPES = ['dossier', 'history_submission', 'downtime_response'];
  ...
  if (type === 'dossier' || type === 'history_submission') {
...
// Query params: type (required), character_id (required), cycle, title
