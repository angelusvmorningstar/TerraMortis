...
function _renderPT(c, m, si, rIdx, mc, dd, editMode, mciPool = 0) {
  ...
  if (editMode) {
    ...
    if (eDots >= 4) {
      const dot4 = m.dot4_skill || '', validAs = as.filter(Boolean).filter(sk => { const s = c.skills?.[sk]; return (s?.dots || 0) < 5; });
      const _pt4Missing = !dot4 || !as.filter(Boolean).includes(dot4);
