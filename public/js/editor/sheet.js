...
export function shRenderInfluenceMerits(c, editMode) {
  ...
  if (editMode) {
    ...
    nonContacts.forEach(m => {
      const idx = inflM.indexOf(m), inf = calcMeritInfluence(m, _inflHWV), tOpts = INFLUENCE_MERIT_TYPES.map(t => '<option' + (m.name === t ? ' selected' : '') + '>' + t + '</option>').join(''), rIdx = c.merits.indexOf(m), dd = (m.cp || 0) + (m.free || 0) + (m.free_mci || 0) + (m.free_vm || 0) + (m.free_lk || 0) + (m.free_ohm || 0) + (m.free_inv || 0) + (m.xp || 0);
...
function _renderMCI(c, m, si, rIdx, mc, dd, editMode) {
  ...
  if (editMode) {
    ...
    if (pool > 0) h += '<div class="mci-pool-row"><span class="mci-pool-lbl">Merit Pool</span><span class="mci-pool-val">' + pool + ' dot' + (pool === 1 ? '' : 's') + '</span><span class="mci-pool-hint">allocate via MCI in any merit or manoeuvre</span></div>';
  } else if (!inactive) {
    const d1c = m.dot1_choice || 'merits', d3c = m.dot3_choice || 'merits', d5c = m.dot5_choice || 'merits';
